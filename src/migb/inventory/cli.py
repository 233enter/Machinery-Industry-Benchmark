...
from .runner import PipelineError, run_d1, run_d2
from .safety import SafetyError


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the MIGB Minimal Corpus Inventory D1/D2 stage")
    ...
    parser.add_argument(
        "--prior-run-id",
        help="prior D1 run ID whose sample must be excluded for D2",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.stage == "d1":
            if args.prior_run_id:
                raise PipelineError("--prior-run-id is only valid for --stage d2")
            result = run_d1(config, repo_root=Path(args.repo_root))
        else:
            if not args.prior_run_id:
